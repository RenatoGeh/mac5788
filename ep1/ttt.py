...
class State:
    ...
    def unique_hash(data):
        h = 0
        for i in data.reshape(BOARD_ROWS * BOARD_COLS):
            if i == -1:
                i = 2
            h = h * 3 + i
        return int(h)
    ...
    def hash(self):
      if self.hash_val is None:
          if SYMMETRIC:
              m = math.inf
              M = np.copy(self.data)
              h = State.unique_hash(M)
              if h in all_hashes:
                self.hash_val = all_hashes[h]
              ...
              self.hash_val = int(m)
              for u in unknowns:
                all_hashes[u] = self.hash_val
          else:
              self.hash_val = State.unique_hash(self.data)
      return self.hash_val
...
if __name__ == '__main__':
    if "--sym" in sys.argv or "-s" in sys.argv:
        SYMMETRIC = True
